...
public class JsonSaveLoadController : MonoBehaviour
{
	...
	private List<IJsonSaveLoad> _coreSaveLoadObjects;
	private List<IJsonSaveLoad> _gameplaySaveLoadObjects;
	...
	public IEnumerator NewGame()
	{
		...
		_fileDataHandler = new JsonFileDataHandler(Application.persistentDataPath, _SAFE_FILE_DATA_TEMP);
	...
	public IEnumerator SaveGame(int saveSlotNumber)
	{
		...
		{
			_fileDataHandler = new JsonFileDataHandler(Application.persistentDataPath, _saveFilePaths[saveSlotNumber - 1]);
		}
	...
	public IEnumerator LoadGame(int loadSlotNumber)
	{
		...
		if (loadSlotNumber != -1)
		{
			...
			StartCoroutine(_gameSceneManager.LoadGameplayScene((GameScenesGameplayEnum)Enum.Parse(typeof(GameScenesGameplayEnum), SceneNameToLoad)));
		...
		if (loadSlotNumber != -1)
		{
			...
			foreach (IJsonSaveLoad coreLoadObj in _coreSaveLoadObjects)
			{
				yield return coreLoadObj.LoadJsonData(_gameData);
		...
		{
			yield return _gameplaySaveLoadObjects[i].LoadJsonData(_gameData);
	...
	public void DeleteGame(int deleteSlotNumber)
	{
		...
		_fileDataHandler = new JsonFileDataHandler(Application.persistentDataPath, _saveFilePaths[deleteSlotNumber - 1]);
	...
	private (string SavefileDateAndTime, GameScenesGameplayEnum SafefileSceneNameSystem) GetExtendedSaveDataForFile(string fileName)
	{
		try
		{
			JsonGameData gameData = _fileDataHandler.LoadFromFile(fileName);
			if (gameData != null)
			{
				return (
